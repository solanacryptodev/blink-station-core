    ...
    <header className="sticky opacity-70 top-0 z-50 flex items-center justify-between w-full h-16 px-4 border-b shrink-0 bg-gradient-to-b from-background/10 via-background/50 to-background/80 backdrop-blur-xl">
      ...
      <div className="flex items-center justify-end space-x-2">
        ...
        <span className="hidden ml-2 md:flex">Settings</span>
          ...
          {wallet.isConnected ? (
              wallet.walletStore.wallet?.name === 'Phantom' && <WalletButton backgroundColor='645E8F' hoverColor='7A73AD' status="Disconnect" /> ||
              wallet.walletStore.wallet?.name === 'Solflare' && <WalletButton backgroundColor='B43E24' hoverColor='B43E24' status="Disconnect" /> ||
              wallet.walletStore.wallet?.name === 'Ledger' && <WalletButton backgroundColor='7F5B32' hoverColor='7F5B32' status="Disconnect" />
          ) : (
